...
namespace Lab3 {
    ...
    Diagram::Diagram(std::string &str) {
        //exceptions
        if (str.length() == 0) {
...
//                break;
    ...
    Diagram &Diagram::concatinate(Diagram &dtmp) {
    ...
    void Diagram::initDiagram(std::istream &s) {
        std::string str;
        s >> str;
        init(str);
    }
    ...
    void Diagram::init(std::string &str) {
        ...
        for (int i = 0; i < str.length();) {
            while (str[i] == tmp && i < str.length()) {
                ++time;
                ++i;
            }//дописать проверку что не выходим за пределы массива
    ...
    [[maybe_unused]] char Diagram::getStatus(int time) const {
        char ch = '\0';
        for (int i = 0; i < this->sum_size; ++i) {
            //
        }
